import { useNavigate } from "react-router-dom";

const IssueCard = ({ issue }) => {
  const navigate = useNavigate();

  const {
    id,
    image,
    title,
    category,
    status,
    priority, // "High" / "Normal"
    location,
    upvotes,
  } = issue;

  const statusColor = {
    Pending: "bg-yellow-100 text-yellow-700",
    "In-Progress": "bg-blue-100 text-blue-700",
    Resolved: "bg-green-100 text-green-700",
    Closed: "bg-gray-200 text-gray-600",
  };

  const priorityColor = {
    High: "bg-red-100 text-red-700",
    Normal: "bg-green-100 text-green-700",
  };

  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition">
      {/* Image */}
      <img
        src={image}
        alt={title}
        className="w-full h-48 object-cover"
      />

      <div className="p-4 space-y-3">
        {/* Title */}
        <h2 className="text-xl font-semibold">{title}</h2>

        {/* Category */}
        <p className="text-gray-500 text-sm">Category: {category}</p>

        {/* Badges */}
        <div className="flex items-center gap-2">
          <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColor[status]}`}>
            {status}
          </span>
          <span className={`px-2 py-1 text-xs font-medium rounded-full ${priorityColor[priority]}`}>
            {priority} Priority
          </span>
        </div>

        {/* Location */}
        <p className="text-sm text-gray-600">
          📍 {location}
        </p>

        {/* Footer */}
        <div className="flex items-center justify-between pt-3">
          {/* Upvote Button */}
          <button className="flex items-center gap-1 px-3 py-1 rounded-lg border hover:bg-gray-100 transition">
            👍 <span>{upvotes}</span>
          </button>

          {/* View Details */}
          <button
            onClick={() => navigate(`/issue/${id}`)}
            className="px-4 py-1 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700 transition"
          >
            View Details
          </button>
        </div>
      </div>
    </div>
  );
};

export default IssueCard;
